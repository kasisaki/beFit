...
type Training struct {
	TrainingType string        // — описывает вид тренировки (Бег, Ходьба, Плавание);
	Action       int           // — описывает количество шагов при беге и ходьбе или количество гребков при плавании;
	LenStep      float64       // — описывает длину одного шага или гребка;
	Duration     time.Duration // — описывает продолжительность тренировки;
	Weight       float64       // — описывает вес пользователя в килограммах.
}
...
// distance возвращает расстояние при плавании.
// Это переопределенный метод distance() из Training.
func (s Swimming) distance() float64 {
	return float64(s.CountPool*s.LengthPool) / MInKm
}
